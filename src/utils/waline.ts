...
const http: AxiosInstance = axios.create({
  baseURL: import.meta.env.WALINE_URL,
  // baseURL: '/',
  timeout: 30 * 1000
})
...
http.interceptors.response.use(
  (response: AxiosResponse<any>) => {
    ...
    if (errno) {
      NProgress.done()
      Notification.error(errmsg || '服务器端错误')
...
request.put = <T = unknown>(url: string, params?: object, headers?: AxiosRequestHeaders): Promise<HttpResponse<T>> => {
  return request({
    method: 'put',
    url,
    data: params,
...
request.delete = <T = unknown>(url: string, params?: object, headers?: AxiosRequestHeaders): Promise<HttpResponse<T>> => {
  return request({
    method: 'delete',
    url,
    data: params,
